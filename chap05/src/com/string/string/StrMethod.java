...
public class StrMethod {
    public static void main(String[] args){
        ...
        boolean equals = hello.equals("你好"); // equals方法判断该字符串是否与目标相等
        System.out.println("是否等于你好="+equals);
        boolean startsWith = hello.startsWith("Hello"); // startsWith方法判断该字符串是否以目标串开头
        System.out.println("是否已Hello开头="+startsWith);
        boolean endsWith = hello.endsWith("World"); // endsWith方法判断字符串是否以目标为结尾
        System.out.println("是否以World结尾="+endsWith);
        boolean contains = hello.contains("or"); // contains方法判断该字符串是否包含目标串
        ...
        int index = hello.indexOf("l"); // indexOf方法返回目标串在源串第一次找到的位置
        System.out.println("首次找到1的位置="+index);
        int lastIndex = hello.lastIndexOf("l"); // lastIndexOf方法返回目标串在源串最后一次找到的位置
        System.out.println("最后找到1的位置="+lastIndex);
        ...
        // 只有一个输入参数的substring方法，从指定位置一致截取到源串的末尾
        String subToEnd = hello.substring(6);
        System.out.println("从第六位截取到末尾="+subToEnd);
        // 有两个输入参数的substring方法，返回从开始位置到结束位置中间截取的字串
