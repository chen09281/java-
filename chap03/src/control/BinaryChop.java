    ...
    public static void main(String[] args){
        ...
        loop:for (int i=0;i<numbers.length;i++){
            ...
            for (int j=0;j<i;j++){ // 遍历数组进行检查，避免塞入重复数字
                if (numbers[j] == item){ // 若已经存在该随机数，则继续第一次层循环，重新生成随机数
                    i--; // 本次循环做了无用功，取消当前的计数
        ...
        Arrays.sort(numbers); // 对整数数组排序(默认升序排序)
        for (int seq=0;seq<numbers.length;seq++){ // 遍历并打印数组中的所有数字
            System.out.println("序号="+seq+",数组="+numbers[seq]);
        }
        ...
        for (int count = 1,position = -1;start<=end;count++){
            ...
            if (numbers[middle]>aim_item){
                ...
                position = middle;
                System.out.println("查找次数="+count+",序号数字="+position);
                break;
            }
        }
    }
